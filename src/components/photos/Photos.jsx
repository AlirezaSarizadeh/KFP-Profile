...
import { fetchPhotoCategories } from '../../api/fetchData';
import GoBackHistory from '../goBackHistory/GoBackHistory';
...
const Photos = () => {
    ...
    useEffect(() => {
        const loadCategories = async () => {
            const data = await fetchPhotoCategories('main');
            setCategories(data);
        };
        loadCategories();
            ...
            <div className="mt-lg-5 mt-5 m-auto row row-cols-lg-3 row-cols-md-3 row-cols-2 w-100">
                ...
                {categories.length ? (
                    categories.map((category) => (
                        ...
                        <Link to={`/dentists/photos/${category.id}`} className="p-2" key={category.id}>
                            <div className="card">
